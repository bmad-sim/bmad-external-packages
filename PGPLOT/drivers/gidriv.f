...
C+
      ...
      REAL    RBUF(*)
      CHARACTER*(*) CHR
C      print *, 'Starting GIDRIV output'
*
...
C
C This gets around the original way of using the C routines grgmem and grfmem to
C allocate memory
C
C
      SAVE UNIT, IC, CTABLE, NPICT, MAXIDX, BX, BY, FILENM
      SAVE CDEFLT, STATE
...
C
  120 CONTINUE
      ...
      IY1=BY-NINT(RBUF(4))
      IF (allocated(pixmap))
     :     CALL GRGI01(IX0, IY0, IX1, IY1, IC, BX, BY)
      RETURN
C
C--- IFUNC=13, Draw dot ------------------------------------------------
C
  130 CONTINUE
      ...
      IY0=BY-NINT(RBUF(2))
      IF (allocated(pixmap))
     :     CALL GRGI01(IX0, IY0, IX0, IY0, IC, BX, BY)
      RETURN
C
...
C
  140 CONTINUE
      IF (UNIT.GE.0) THEN
         CALL GRGI06(UNIT, BX, BY, CTABLE, MAXIDX)
         JUNK = GRCFIL(UNIT)
      END IF
      NPICT = NPICT+1
...
C
  240 CONTINUE
      ...
      IY0=BY-NINT(RBUF(4))
      IF (allocated(pixmap)) 
     :     CALL GRGI03(IX0, IY0, IX1, IY1, IC, BX, BY)
      RETURN
C
...
C
  260 CONTINUE
      CALL GRGI04(NBUF, RBUF, BX, BY, MAXIDX)
      RETURN
C
...
*+
      SUBROUTINE GRGI01 (IX0, IY0, IX1, IY1, ICOL, BX, BY)
      use pgplot_mod
...
*+
      SUBROUTINE GRGI03 (IX0, IY0, IX1, IY1, ICOL, BX, BY)
      use pgplot_mod
      INTEGER IX0, IY0, IX1, IY1
      INTEGER ICOL, BX, BY
*
...
*+
      SUBROUTINE GRGI04(NBUF,RBUF,BX,BY,MAXIDX)
      use pgplot_mod
...
*+
      SUBROUTINE GRGI06 (UNIT, BX, BY, CTABLE, MAXIDX)
      use pgplot_mod
