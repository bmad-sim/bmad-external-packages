SUBROUTINE ZGGSVD3_F95( A, B, ALPHA, BETA, K, L, U, V, Q, IWORK, INFO )
!
...
!  .. USE STATEMENTS ..
   ...
   USE F77_LAPACK, ONLY: GGSVD3_F77 => LA_GGSVD3
!  .. IMPLICIT STATEMENT ..
   IMPLICIT NONE
!  .. SCALAR ARGUMENTS ..
...
! =======
! 
!      LA_GGSVD3 computes the generalized singular values and, optionally,
...
!     LA_GGSVD3 computes the real (nonnegative) scalars alpha(i), beta(i),
! i=1,2,..., k+l , the matrix R, and, optionally, the transformation 
! matrices U , V and Q.
! 
! =========
! 
!      SUBROUTINE LA_GGSVD3( A, B, ALPHA, BETA, K=k, L=l, &
...
!----------------------------------------------------------------------
!  .. LOCAL PARAMETERS ..
   CHARACTER(LEN=8), PARAMETER :: SRNAME = 'LA_GGSVD3'
!  .. LOCAL SCALARS ..
    ...
    ENDIF
      ...
      IF( ISTAT == 0 ) THEN
         IF( PRESENT(U) )THEN
           IF( PRESENT(V) )THEN
             IF( PRESENT(Q) )THEN
                  CALL GGSVD3_F77( LJOBU, LJOBV, LJOBQ, M, N, P, LK, LL, A, MAX(1,M), &
                         B, MAX(1,P), ALPHA, BETA, U, MAX(1,S1U), &
                        V, MAX(1,S1V), Q, MAX(1,S1Q), WORK, LWORK, RWORK, LIWORK, LINFO )
		ELSE
		  CALL GGSVD3_F77( LJOBU, LJOBV, LJOBQ, M, N, P, LK, LL, A, MAX(1,M), &
                         B, MAX(1,P), ALPHA, BETA, U, MAX(1,S1U), &
                        V, MAX(1,S1V), LLQ, MAX(1,S1Q), WORK, LWORK, RWORK, LIWORK, LINFO )
		ENDIF
	       ELSE
	        IF( PRESENT(Q) )THEN 
		   CALL GGSVD3_F77( LJOBU, LJOBV, LJOBQ, M, N, P, LK, LL, A, MAX(1,M), &
                         B, MAX(1,P), ALPHA, BETA, U, MAX(1,S1U), &
                        LLV, MAX(1,S1V), Q, MAX(1,S1Q), WORK, LWORK, RWORK, LIWORK, LINFO )
		ELSE
		  CALL GGSVD3_F77( LJOBU, LJOBV, LJOBQ, M, N, P, LK, LL, A, MAX(1,M), &
                         B, MAX(1,P), ALPHA, BETA, U, MAX(1,S1U), &
                        LLV, MAX(1,S1V), LLQ, MAX(1,S1Q), WORK, LWORK, RWORK, LIWORK, LINFO )
		ENDIF
	       ENDIF
	       ELSE
	         IF( PRESENT(V) )THEN
